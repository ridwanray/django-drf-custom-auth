...
class TestPermissions:
    ...
    def test_retrieve_permissions_list(self, api_client, permission_factory, authenticate_user):
        permission_factory.create_batch(3)
    ...
    def test_retrieve_permission_detail(self, api_client, permission_factory, authenticate_user):
        permission = permission_factory(name="Permission1")
        user = authenticate_user(is_admin=True)
        token = user['token']
        api_client_with_credentials(token, api_client)
        url = reverse("permission:permission-detail",
                      kwargs={"pk": permission.id})
        response = api_client.get(url)
        assert response.status_code == 200
        assert response.json()['name'] == 'Permission1'

    def test_retrieve_permissions_by_role(self, api_client, permission_factory, authenticate_user):
        permissions = permission_factory.create_batch(4)
        permission_factory.create_batch(3)
        role = RoleFactory(name="Role1")
        role.permissions.set(permissions)
        user = authenticate_user(is_admin=True)
        token = user['token']
        api_client_with_credentials(token, api_client)
        url = reverse("role:role-get-permissions-by-role",
                      kwargs={"pk": role.id})
        response = api_client.get(url)
        assert response.status_code == 200
        assert len(response.json()['data']) == 4

    def test_deny_retrieve_for_nonadmin(self, api_client, permission_factory, authenticate_user):
        permission_factory.create_batch(3)
...
class TestRoles:
    role_list_url = reverse('role:role-list')

    def test_admin_create_roles(self, api_client, permission_factory, authenticate_user):
        permission_factory = permission_factory()
        user = authenticate_user(is_admin=True)
        token = user['token']
        api_client_with_credentials(token, api_client)
        data = {
            "permissions": [permission_factory.id],
            "name": "string",
        }

        response = api_client.post(self.role_list_url, data)
        returned_json = response.json()
        assert response.status_code == 201
        assert 'id' in returned_json
        assert returned_json['name'] == data['name']
        assert len(returned_json['permissions']) == 1

    def test_retrieve_roles(self, api_client, seed_roles, authenticate_user):
        seed_roles(role_names=["Programs Manager", "Business Operations"])
        user = authenticate_user(is_admin=True)
        token = user['token']
        api_client_with_credentials(token, api_client)
        response = api_client.get(self.role_list_url)
        assert response.status_code == 200
    ...
    def test_delete_roles(self, api_client, seed_roles, authenticate_user):
        ...
        api_client_with_credentials(token, api_client)
        url = reverse("role:role-detail",
                      kwargs={"pk": seeded_roles[0].id})
        response = api_client.delete(url)
        assert response.status_code == 204

    def test_update_roles(self, api_client, permission_factory, authenticate_user):
        user = authenticate_user(is_admin=True)
        token = user['token']
        api_client_with_credentials(token, api_client)
        permissions = permission_factory.create_batch(4)
        role = RoleFactory(name="Role1")
        role.permissions.set(permissions)
        assert role.permissions.count() == 4

        data = {
            "name": "Role1 Updated",
            "permissions": [permissions[0].id],
        }
        url = reverse("role:role-detail", kwargs={"pk": role.id})
        response = api_client.patch(url, data)
        assert response.status_code == 200
        returned_json = response.json()
        assert returned_json['name'] == data['name']
        assert len(returned_json['permissions']) == len(data['permissions'])

        role.refresh_from_db()
        assert role.permissions.count() == len(data['permissions'])
